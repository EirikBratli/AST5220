module evolution_mod
  ...
  ! For high resolution in Source function
...
contains
  ...
  subroutine get_hires_source_function(x, k, S)
    implicit none

    real(dp), pointer, dimension(:),   intent(out) :: k, x   ! pointer
    real(dp), pointer, dimension(:,:), intent(out) :: S      ! Source function to return, pointer

    integer(i4b) :: i, j, n_hires, ii
    real(dp)     :: g, dg, ddg, tau, dtau, ddtau, Hp, dHp, ddHHp, Pii, dPi, ddPi
    real(dp)     :: ck, ckHp, dHgPi, a, dHdHp
    ...
    allocate(x(n_hires))
    allocate(k(n_hires))
    allocate(S_lores(1:n_t, 1:n_k))      ! Low resolution
    allocate(Coeff(4, 4, n_t, n_k))
    allocate(S(n_hires, n_hires)) ! High resolution
    allocate(xx(501))
    
    print *,'   - Compute S'
    ! Generate high resolution grids for k and x:
    do i=1, 501
       xx(i) = x_t(i+499)
    end do
    
    do i=1, n_hires
      !x(i) = x_init - x_init*(i-1.d0)/(n_hires-1.d0)
      k(i) = k_min + (k_max-k_min)*((i-1.d0)/(n_hires-1.d0))
    end do
    do i=1, n_hires
       x(i) = xx(1) + (0.d0-xx(1))*(i-1.d0)/(n_hires-1.d0)
       !print *, i, x(i), k(i)
    end do
    ...
    do i=1, n_t
       ...
       tau    = get_tau(xx(i))
       dtau   = get_dtau(xx(i))
       ddtau  = get_ddtau(xx(i))
       Hp     = get_H_p(xx(i))
       dHp    = get_dH_p(xx(i))
       a      = exp(xx(i))

       dHdHp  = 2*Hp*dHp + 0.5d0*H_0**2 * (3.d0*(Omega_m+Omega_b)/a + 8.d0*Omega_r/a**2)
       ...
       do j=1, n_k
          ...
          ! Terms in S(x,k)
          ddPi  = 0.4d0*ckHp*(-dHp*Theta(ii,1,j)/Hp + dTheta(ii,1,j)) &
                + 0.3d0*(ddtau*Pii + dtau*dPi) &
                - 0.6d0*ckHp*(-dHp*Theta(ii,3,j)/Hp + dTheta(ii,3,j))
          ! The 2nd derivative of the last term in S
          dHgPi = dHdHp *g*Pii + 3.d0*Hp*dHp*(dg*Pii + g*dPi) &
                + Hp**2*(ddg*Pii + 2.d0*dg*dPi + g*ddPi)
          
          ! Compute low resolution S:
          S_lores(i,j) = g*(Theta(ii,0,j) + Psi(ii,j) + 0.25d0*Pii) &
                       + exp(-tau)*(dPsi(ii,j) - dPhi(ii,j)) &
                       - (1.d0/ck)*(dHp*g*v_b(ii,j) + Hp*dg*v_b(ii,j) + Hp*g*dv_b(ii,j)) &
                       + (0.75d0/ck**2)*dHgPi
          
          !if (j==59) then
          !   print *, g*(Theta(ii,0,j) + Psi(ii,j) + 0.25d0*Pii)
          !end if 
    ...
    call splie2_full_precomp(xx, ks, S_lores, coeff)
    ...
    do i=1, n_hires
       !print *, x(i)
       do j=1, n_hires
          S(i,j) = splin2_full_precomp(xx, ks, coeff, x(i), k(j))
       end do
    end do
  ...
  subroutine integrate_perturbation_eqns
    ...
    do k = 1, n_k
       ...
       do i = j_tc-2, n_t
          ...
          dPsi(i,k)     = -dPhi(i,k) - 12.d0*(H_0/(ck))**2 &
                        * Omega_r * (dTheta(i,2,k) - 2.d0*Theta(i,2,k))*exp(-2.d0*x_t(i))  
  ...
  subroutine dfdx(x, y_tc, dydx)
    ...
    Psi       = -Phi - 12.d0*Omega_r*Theta2*(H_0/(c*k*a))**2 !* Omega_r*Theta2 
    
   
    ! Calculate derivatives:
    !dPhi
    dydx(5) = Psi - (ckHp**2/3.d0)*Phi + (H_0**2/(2.d0*Hp**2)) & 
            * (Omega_m*delta/a + Omega_b*delta_b/a + (4.d0*Omega_r/a**2)*Theta0)
    ! d_delta
  ...
  subroutine derivs(x, y, dydx)
    ...
    R        = (4.d0*Omega_r)/(3.d0*Omega_b*a)
    Psi      = -Phi - 12.d0*Omega_r*Theta2*(H_0/(c*k_current*a))**2 !* Omega_r*Theta2

    ! Derivation
    
    ! dPhi       
    dydx(5) = Psi - ((ckHp**2)/3.d0)*Phi + (H_0**2/(2.d0*Hp**2)) &
            * ( (Omega_m/a)*delta + (Omega_b/a)*delta_b + 4.d0*Omega_r*Theta0/a**2 )
    ! d_delta  
    dydx(1) = ckHp*v - 3.d0*dydx(5)
    ! d_delta_b 
    dydx(2) = ckHp*vb - 3.d0*dydx(5)
    ...
    ! dTheta2
    dydx(8) = 0.4d0*ckHp*Theta1 - 0.6d0*ckHp*Theta3 + 0.9d0*dtau*Theta2
    !dTheta_l    
    do l=3, lmax_int-1       
  ...
  subroutine write_to_file_mk3
    ...
    open(11, file='derivatives.dat', status='replace')
    ...
    do i=1, n_t
       ...
       !do j=1,6
       !   write(11 , '(7(ES15.5E3,1x))') dPhi(i, kv(j)), dPsi(i, kv(j)), dv_b(i,kv(j)), &
       !        dTheta(i,0,kv(j)), dTheta(i,1,kv(j)), dTheta(i,2,kv(j)), dTheta(i,3,kv(j))
               !&, dTheta(i,4,kv(j)), dTheta(i,5,kv(j)), dTheta(i,6,kv(j)) 
       !end do   
    end do   

    do i=1, n_t
       write(11, '(7(ES15.5E3,1x))') dPhi(i,59), dPsi(i,59), dv_b(i, 59), dTheta(i,0,59), dTheta(i,1,59), dTheta(i,2,59), dTheta(i,3,59)  
    end do
